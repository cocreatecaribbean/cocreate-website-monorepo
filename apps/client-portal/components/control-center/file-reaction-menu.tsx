...
export type FileReactionMenuProps = {
  attachmentId: string
...
export default function FileReactionMenu({
  ...
  const applyReaction = async (kind: ProjectFileReactionKind) => {
    ...
    setReaction(kind)
    const result = await setFileReaction(attachmentId, kind)
    setSaving(false)
    if (result) {
  ...
  const removeReaction = async () => {
    ...
    const previous = reaction
    setReaction(null)
    const result = await clearFileReaction(attachmentId)
    setSaving(false)
    if (result) {
      setReaction(result.myReaction)
      onChange?.(result)
          ...
          {REACTION_OPTIONS.map((option) => (
            ...
            >
              ...
              {option.isPositive ? (
                ...
                <span className="h-1.5 w-1.5 rounded-full bg-casablanca" aria-hidden />
              )}
            </button>
          ))}
          {reaction ? (
