...
function formatRouteContext(route?: PortalRouteContext): string {
  ...
  return `CURRENT LOCATION: The user is currently on ${route.pathname}${search}. Prefer guidance relative to this screen when helpful.`
}
...
Be warm, concise, and practical. Point people to the exact nav item and path.
...
${location ? `\n${location}\n` : ''}`
  ...
  if (!retrieved) {
    return `${base}
If PRODUCT FACTS do not cover the question, say you are not sure and suggest Get Help (/messages), Team, or a Super admin.`
  }
